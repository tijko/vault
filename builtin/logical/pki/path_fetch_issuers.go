...
func pathListIssuers(b *backend) *framework.Path {
	return &framework.Path{
		...
		Callbacks: map[logical.Operation]framework.OperationFunc{
			logical.ListOperation: b.pathListIssuers,
		},
...
func (b *backend) pathListIssuers(ctx context.Context, req *logical.Request, data *framework.FieldData) (*logical.Response, error) {
...
func pathGetIssuer(b *backend) *framework.Path {
	return &framework.Path{
		...
		Pattern: "issuer/" + framework.GenericNameRegex("ref"),
...
func pathGetRawIssuer(b *backend) *framework.Path {
	return &framework.Path{
		// Returns the certificate in raw PEM or DER form.
		...
		Callbacks: map[logical.Operation]framework.OperationFunc{
			logical.ReadOperation: b.pathGetRawIssuer,
		},
